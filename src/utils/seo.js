export const getSeoMeta = () => {
  return {
    title: "Consultoría en Cultura Organizacional y Liderazgo | Escala",
    description:
      "Instalamos una cultura que crea líderes autónomos, equipos con ownership real y cumplimiento consistente de KPIs. Especialistas en metodologías de alto rendimiento para PYMES.",
    keywords:
      "organizational culture consulting, leadership consulting, high performance teams, business scaling consulting, KPI consulting, organizational transformation, leadership systems for companies, pymer",
    url: "https://escala.uno",
    image: "https://escala.uno/og-image.jpg", // Placeholder for real image
  };
};
